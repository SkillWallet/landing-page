...
export const joinCommunity = async (communityAddress, username, skill, level) => {
  try {
    ...
    console.log(url);
    const createTx = await contract.joinNewMember(url, 2006);
...
export const fetchSkillWallet = async (address) => {
...
export const activatePA = async (partnersAddress) => {
  try {
    const provider = new ethers.providers.Web3Provider(window.ethereum);
    const signer = provider.getSigner();
    const contract = new ethers.Contract(partnersAddress, JSON.stringify(partnersAbi), signer);
